...
SELECT DISTINCT ON (cs.channel_id, cs.total_sales)
    cs.channel_desc,
    cs.cust_last_name,
    cs.cust_first_name,
    -- format total sales amount as a readable string with two decimal places
    TO_CHAR(cs.total_sales, 'FM999999990.00') AS amount_sold,
    -- calculate and format the sales percentage for the customer within their channel
    TO_CHAR(
...
FROM (
    SELECT 
        ...
        ch.channel_id,
        ch.channel_desc,
        -- Aggregate sales amount per customer and channel
        SUM(sa.amount_sold) AS total_sales
    FROM 
        sales sa
        JOIN customers cu ON sa.cust_id = cu.cust_id  -- Join to get customer details
        JOIN channels ch ON sa.channel_id = ch.channel_id  -- Join to get channel details
    GROUP BY 
        cu.cust_id, cu.cust_first_name, cu.cust_last_name, ch.channel_id, ch.channel_desc
) cs

-- I joined with another subquery to calculate total sales per channel
JOIN (
    SELECT 
        ch.channel_id,
        -- Sum all sales per channel to calculate percentage contribution later
...
-- I ordered by channel and descending sales to ensure top sellers per channel come first
ORDER BY 
    cs.channel_id, cs.total_sales DESC

-- limit the result to top 100 rows (I did this because loading took too much time so this is faster )
LIMIT 100;
...
SELECT 
    ...
    -- calculating the total sales amount for each product category per month
    SUM(s.amount_sold) AS total_sales
-- Join necessary tables to link sales data with time and product details
FROM 
    sales s
JOIN 
    times t ON s.time_id = t.time_id  -- join to get calendar month description
JOIN 
    products p ON s.prod_id = p.prod_id  -- join to categorize sales by product type
-- filtering the data to include only the first three months of the year 2000
WHERE 
    t.calendar_month_desc IN ('2000-01', '2000-02', '2000-03')
-- group results by month and product category to prepare for aggregation
GROUP BY 
    t.calendar_month_desc,
    p.prod_category
-- order the final results first by month, then by category alphabetically
ORDER BY 
    month,
