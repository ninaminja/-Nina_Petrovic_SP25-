...
WITH daily_sales AS (
    SELECT 
        ...
        t.time_id,
        t.day_name,
        COALESCE(SUM(s.amount_sold), 0) AS sales --to set sales = 0 if there are no sales
    FROM 
        times t
        LEFT JOIN sales s ON t.time_id = s.time_id --LEFT JOIN with the sales table, so as not to exclude days without sales
    WHERE --it takes all days from the 49th, 50th and 51st week of 1999
    ...
    day_name,
    sales,
    --this gives the total of sales from the beginning of the week to that day (window function)
    SUM(sales) OVER (
        PARTITION BY calendar_week_number 
        ORDER BY time_id --ORDER BY time 
    ) AS cum_sum,
    CASE    --centered 3-day average with special cases for Monday/Friday
        WHEN day_name = 'Monday' THEN -- there is no "previous day" in the week, so Saturday and Sunday of the previous week + Monday + Tuesday (4 days) are used
            ROUND((
                LAG(sales, 2) OVER (ORDER BY time_id) +  -- Saturday (prev week)
                LAG(sales, 1) OVER (ORDER BY time_id) +  -- Sunday (prev week)
                sales +                                  -- Monday
                LEAD(sales, 1) OVER (ORDER BY time_id)   -- Tuesday
            ) / 4, 2)
            
        WHEN day_name = 'Friday' THEN --use Thursday, Friday, Saturday and Sunday (4 days)
            ROUND((
        ...
        ELSE -- regular 3-day centered average
            ROUND((
